...
export interface ImageGenProgressState {
   ...
   *   that returns the full-resolution result is still in flight. We retain the
   *   last preview frame during this window so the UI bridges the gap between
   *   the final preview frame and the final media result instead of blanking out.
   */
  phase: ImageGenPhase
}
 ...
 * callers can keep showing something until the final result is ready; it resets
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      wsClient.on(EventType.IMAGE_GEN_COMPLETE, (payload: { assetId: string }) => {
        if (payload.assetId !== assetId) return
        // The model is done, but the HTTP call that returns the full result is
        ...
        // result (which also unmounts this subscription).
